...
if __name__ == '__main__':
    ...
    while (currentEpoch <= args.epochs):
        ...
        while (len(flagList) != 0):
            ...
            for item in list(flagList):  # Use list() to avoid modification during iteration
                try:
                    ...
                    if parasInfoR.strip():  # Check if response is not empty
        ...
        for item in deviceName:
            ...
            if os.path.exists(localFileName):
                try:
                    w_locals.append(torch.load(localFileName))
                except Exception as e:
                    print(f"Error loading {item} model: {e}")
            else:
                print(f"Warning: Model file not found for {item} (lazy node or timeout)")
