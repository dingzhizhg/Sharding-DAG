...
def modelBuild():
    ...
    if args.dataset == 'mnist':
        # Use num_channels from args to determine data transformation
        if args.num_channels == 1:
            # Single channel transformation for MNIST
            trans_mnist = transforms.Compose([
                transforms.ToTensor(),
                transforms.Normalize((0.1307,), (0.3081,))
            ])
        elif args.num_channels == 3:
            # Three channel transformation for MNIST (repeat channels)
            trans_mnist = transforms.Compose([
                transforms.ToTensor(),
            ...
            raise ValueError(f"Unsupported num_channels: {args.num_channels}. Must be 1 or 3 for MNIST.")
